        ...
        <div class="detail-grid">
            <section class="action-card">
                ...
                <div class="form-body">
                    ...
                    <div class="input-row">
                        <div class="input-group"><label>Miktar</label><input v-model="form.amount" type="number" class="custom-input" placeholder="0.00"></div>
...
<script setup>
    ...
    const totalInSelectedUnit = computed(() => {
        ...
        return sum.toLocaleString('tr-TR', { minimumFractionDigits: 2 });
    });
...
<style scoped>
    .detail-container { min-height: 100vh; padding: 40px; color: white; font-family: 'Inter', sans-serif; box-sizing: border-box; background: #0f172a; transition: background 0.3s ease; }
    ...
    .dd-btn { background: #1e293b; color: white; border: 1px solid #334155; padding: 10px 20px; border-radius: 12px; cursor: pointer; font-weight: 700; min-width: 160px; display: flex; justify-content: space-between; align-items: center; }
    .dd-menu { position: absolute; top: 100%; right: 0; background: #1E293B; border: 1px solid #334155; border-radius: 12px; width: 220px; z-index: 100; max-height: 400px; overflow-y: auto; box-shadow: 0 10px 40px rgba(0,0,0,0.5); margin-top: 10px; }
    .dd-item { padding: 12px 15px; cursor: pointer; border-bottom: 1px solid rgba(255,255,255,0.05); color: white; }
    .dd-item:hover { background: #334155; color: #FFD700; }
    .icon-btn { background: #1e293b; color: white; border: 1px solid #334155; width: 45px; height: 45px; border-radius: 10px; cursor: pointer; font-size: 1.2rem; display: flex; align-items: center; justify-content: center; transition: 0.3s; }
    .hero-container { display: flex; justify-content: center; margin-bottom: 50px; }
    .hero-card { background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(15px); border: 1px solid rgba(255, 255, 255, 0.1); padding: 30px 60px; border-radius: 24px; text-align: center; min-width: 320px; }
    ...
    .action-card, .history-card { background: rgba(0,0,0,0.3); backdrop-filter: blur(20px); border-radius: 24px; border: 1px solid rgba(255,255,255,0.1); }
    .form-body { padding: 30px; display: flex; flex-direction: column; gap: 20px; }
    .custom-input { width: 100%; padding: 16px; background: rgba(0,0,0,0.4); border: 1px solid rgba(255,255,255,0.15); color: white; border-radius: 12px; font-size: 1.1rem; }
    .action-buttons { display: flex; gap: 20px; }
    .btn-add, .btn-sub { flex: 1; padding: 18px; border-radius: 16px; font-weight: 800; cursor: pointer; border: none; color: white; transition: 0.3s; }
    ...
    .receipt-btn { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); width: 40px; height: 40px; border-radius: 10px; cursor: pointer; color: white; }
</style>